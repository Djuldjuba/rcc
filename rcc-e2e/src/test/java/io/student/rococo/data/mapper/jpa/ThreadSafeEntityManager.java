package io.student.rococo.data.mapper.jpa;
...
public class ThreadSafeEntityManager implements EntityManager {
    ...
    public ThreadSafeEntityManager(EntityManager delegate, String jdbcUrl) {
        this.threadEm.set(delegate);
        this.emf = delegate.getEntityManagerFactory();
        this.jdbcUrl = jdbcUrl;
        this.threadId = String.valueOf(Thread.currentThread().threadId());

        try {
            Class<?> databaseExt = Class.forName("io.student.rococo.jupiter.extensions.DatabaseExtension");
            databaseExt.getMethod("registerEntityManager", EntityManager.class, String.class)
                    .invoke(null, this, jdbcUrl);
        } catch (Exception e) {
    ...
    public void close() {
        ...
        if (em != null && em.isOpen()) {
            try {
                ...
                try {
                    Class<?> databaseExt = Class.forName("io.student.rococo.jupiter.extensions.DatabaseExtension");
                    databaseExt.getMethod("unregisterEntityManager", EntityManager.class, String.class)
                            .invoke(null, this, jdbcUrl);
                } catch (Exception ignored) {
                }
                threadEm.remove();
